...
When the family was 6 months of age, 50 fish were sacrificed and had their brains removed. Genotyping was performed by PCR then 24 fish of equal Genotypes and sex had brain total RNA extracted and DNase I treatment. Then total RNA was sent to SAHMRI for RNA-seq (single ended, 75bp insert). The resulting fastq files were then processed using bash scripts to produce a counts matrix (kallisto was used to estimate transcript abundances). Kallisto settings were mean frag length = 300, sd = 60, 50 bootstraps. None of the sorl1 alternate transcripts were added to the transcriptome as *sorl1* is a long gene, and kallisto couldnt tell the alternate transcripts apart in a previous draft analysis, 
...
The code below performs the tests for differential experssion using liklihood ratio tests from `edgeR`.
...
The scrollboxes below contain the toptables. Not very many DE genes are observed in each comparisom. 
...
The scrollboxes below contain the entire toptables. More DE genes are observed in each comparison now. 
...
labels = "AUTO"
  ...
  ggsave("plots/celltype_heatmaps/celltype_logCPM_violin.png", width = 17, height = 17, units = "cm", dpi = 200)
```
...
legend = "bottom"
  ...
  ggsave("plots/gc_lengthplots.png", width = 15, height = 15, units = "cm", dpi = 300)
...
ggarrange(
  ...
  common.legend = TRUE, 
  legend = "bottom") +
  ggsave("plots/PCA_all_v1.png", width = 18, height = 18, units = "cm", dpi = 300)
```
